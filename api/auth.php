...
require_once '../includes/functions.php';
 ...
 if(isset($_GET['username'],$_GET['password'])){
 	...
 	$device_id = isset($_GET['device_id']) ? strip_tags(trim($_GET['device_id'])) : '';
 	$device_model = isset($_GET['device_model']) ? strip_tags(trim($_GET['device_model'])) : 'Unknown';
 	...
 	if(empty($_GET['username'])){
        ...
        $premuim_duration = date('Y-m-d h:i:s', $premuim_duration);
 		...
 		if ($auth_check){
 		        ...
 		        $sql2 = $db->sql_query("SELECT device_id, device_model FROM users WHERE user_name='$username' AND auth_vpn=md5('$password')");
 		        $result2 = $db->sql_fetchrow($sql2);
 		        
 		        $dev_id = isset($result2['device_id']) ? $result2['device_id'] : '';
 		        $dev_model = isset($result2['device_model']) ? $result2['device_model'] : 'Unknown';
