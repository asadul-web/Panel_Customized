...
if(!isset($_GET['uid']) || empty($_GET['uid'])){
	...
	$blink = isset($row_bio['bio_link']) ? $row_bio['bio_link'] : '';
	
	// Check if bio file exists before trying to open it
	$content = 'I am a freelance based in Saudi Arabia, passionate about career development and helping people find new roles.';
	$bioFilePath = $_SERVER['DOCUMENT_ROOT'] . "/profile/$uid/$blink";
	if(!empty($blink) && file_exists($bioFilePath)){
	    $file = fopen($bioFilePath, "r");
	    if($file !== false){
	        $content_ = fread($file, filesize($bioFilePath));
	        if(!empty($content_)){
	            $content = $content_;
	        }
	        fclose($file);
	...
	// Initialize user_level variable
	$user_level = 'Unknown';
	if($userlevel == 'normal'){
	    $user_level = 'Normal';
	...
	$prof2_image = isset($row2['profile_image']) ? $row2['profile_image'] : '';
