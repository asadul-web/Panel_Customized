...
if($user_id_2 == 1 || $user_level_2 == 'superadmin' || $user_level_2 == 'developer' || $user_level_2 == 'reseller'){
    ...
    if(isset($_POST['submitted'])  == 'change_password'){
        $valid = true;
        if(empty($newpass)){
            $errormsg[] = 'New password is empty.<br>';
            $valid = false;
        }
        
        if(preg_match('/[^_a-z-A-Z-0-9 ]/', $newpass)){
            $errormsg[] = 'Only numbers and letters are allowed in new user password.<br>';
            $valid = false;
        }
        
        if(strlen($newpass) < 5){
            $errormsg[] = 'New password too short minimum 5 letters/numbers.<br>';
            $valid = false;
        }
