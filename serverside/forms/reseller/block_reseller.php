...
if($user_id_2 == 1 || $user_level_2 == 'superadmin' || $user_level_2 == 'developer' || $user_level_2 == 'reseller'){
	...
	$confirmblock = $db->Sanitize(trim($_POST['confirmblock']));
    
    $qry = $db->sql_query("SELECT user_name, upline FROM users WHERE user_id='$uid'");
    $row = $db->sql_fetchrow($qry);
    $username = $row['user_name'];
    $userupline = $row['upline'];
    
    if(isset($_POST['submitted'])  == 'block_user'){
        $valid = true;
        ...
        if($valid){
            ...
            {
                if($confirmblock == 'doblock'){
                    $block_downline = $db->sql_query("UPDATE users SET is_freeze=1 WHERE upline='$uid'");
                    if($block_downline){
                        $update = $db->sql_query("UPDATE users SET is_freeze=1 WHERE user_id='$uid'");
                        $action = 'User <code>'.$username.'</code> was blocked.';
                        ...
                        if($update && $addactivity){
                            $success_message = 'User <code>'.$username.'</code> blocked successfully!';
                            $values['response'] = 1;
                            $values['msg'] = $success_message;
                        }else{
                            $error_message = 'User <code>'.$username.'</code> blocked failed!';
                            $values['response'] = 2;
                ...
                }elseif($confirmblock == 'dounblock'){
                    $unblock_downline = $db->sql_query("UPDATE users SET is_freeze=0 WHERE upline='$uid'");
                    if($unblock_downline){
                        $update = $db->sql_query("UPDATE users SET is_freeze=0 WHERE user_id='$uid'");
                        $action = 'User <code>'.$username.'</code> was unblocked.';
                        $addactivity = $db->sql_query("INSERT INTO activity_logs 
            							(user_id, date, action, ipaddress, device_os, device_client) 
            							values
            							('$user_id_2', '".date('Y-m-d H:i:s')."', '$action', '".$_SERVER['REMOTE_ADDR']."','$deviceOS','$device_client')");
                        if($update && $addactivity){
                            $success_message = 'User <code>'.$username.'</code> unblocked successfully!';
                            $values['response'] = 1;
                            $values['msg'] = $success_message;
                        }else{
                            $error_message = 'User <code>'.$username.'</code> unblocked failed!';
                            $values['response'] = 2;
                            $values['msg'] = $error_message;
                        }
                    }else{
