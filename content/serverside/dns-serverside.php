...
$totalDataQuery = $db->sql_query("SELECT * FROM dns") or die();
$totalData = $db->sql_numrows($totalDataQuery);

if( !empty($requestData['search']['value']) ) { 
	$searchValue = $db->Sanitize($requestData['search']['value']);
	$sql.=" AND ( host_name LIKE '%".$searchValue."%' "; 
	$sql.=" OR domain_name LIKE '%".$searchValue."%' ";
	$sql.=" OR ip_address LIKE '%".$searchValue."%' ) ";
}
