...
abstract class Smarty_Internal_CompileBase
{
    ...
    /**
     * PHP 8 compatible replacement for deprecated each() function
     */
    private function smarty_each(&$array) {
        $key = key($array);
        $value = current($array);
        next($array);
        if ($key !== null) {
            return array('key' => $key, 'value' => $value, 1 => $value, 0 => $key);
        }
        return false;
    }
